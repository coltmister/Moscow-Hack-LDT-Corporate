...
class UserSessionsView(APIView):
    ...
    def get(self, request, user_uuid=None, *args, **kwargs):
        user = kwargs['user']
        if user_uuid:
            try:
                user = User.objects.get(id=user_uuid)
        ...
        return Response(status=200, content=sessions or [])

    def delete(self, request, user_uuid, session_id, *args, **kwargs):
        try:
            user = User.objects.get(id=user_uuid)
        except ObjectDoesNotExist:
            return Response(status=400, content="Пользователь не найден")
        ...
        if str(session_id) not in session_ids:
            return Response(status=400, content=f"Данная сессия не принадлежит выбранному пользователю")
        ...
        try:
            ...
            telegram_message(message)
            logger.warning(message)
            return Response(status=400, content=f"Не удалось удалить сессию, попробуйте повторить позднее")
...
@method_decorator([tryexcept, auth, log_action], name='dispatch')
class MySessionDevicesView(APIView):
    keycloak = None
    ...
    def get(self, request, *args, **kwargs):
        user = kwargs['user']
        user_access_token = kwargs['access_token']
        try:
            devices = self.keycloak.get_user_devices(user_access_token=user_access_token)
        except KeycloakResponseException as error:
            message = f"Не удалось получить список устройств пользователя {user.id}. Ошибка {error}"
            telegram_message(message)
            logger.warning(message)
            return Response(status=400, content=f"Не удалось получить список устройств, попробуйте повторить позднее")
        return Response(status=200, content=devices or [])
